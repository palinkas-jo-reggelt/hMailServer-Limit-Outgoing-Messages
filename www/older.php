...
<?php
	...
	if ($search==""){
		...
		$search_SQL = " AND account LIKE '%".$search."%'";
	}

	$total_pages_sql = $pdo->prepare("
		SELECT Count( * ) AS count 
		FROM hm_accounts_mobile 
		WHERE DATE(lastmessagetime) < DATE(NOW())".$search_SQL
	);
	...
	// echo "<br><br>Table does not include accounts that have never logged a sent message";
?>
